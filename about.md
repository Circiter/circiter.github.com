...
менее, ссылки могут указывать на объекты, определённые как выше, так и ниже по тексту.
...
включении режима `shared_context`, все формулы одного `.md`-файла собираются в единый файл 
`.tex`. Это, правда, влечёт за собой некоторые нежелательные побочные эффекты (по крайней мере 
сейчас ), вроде недоступности кэширования уже сверстанных формул или необходимость в 
двупроходной обработке исходного текста, но зато позволяет обращаться с формулами в 
...
дважды, в то время как для режима `shared_context: false` достаточно одного прохода (но зато 
аж для всех формул... территория компромиссов указывает на оптимальность решения, иногда :) ).
