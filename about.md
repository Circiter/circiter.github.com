...
блог-посты); этот плагин использует всем `*.bib`-файлы (для $\mbox{\normalfont 
B\kern-.05em{\scshape i\kern-0.25em b}\kern-.08em \TeX}$) в качестве базы данных 
библиографических записей, но для описания стилей оформления записей и ссылок используются не 
`*.bst`, а `*.csl`-файлы.
