...
Плагин `jekyll-scholar` используется для верстки библиографических списков и 
внутритекстовых ссылок на них (пока я только начинаю использовать этот плагин, постепенно 
адаптируя имеющиеся блог-посты); этот плагин использует всем знакомые `*.bib`-файлы в качестве 
базы данных библиографических записей, но для описания стилей оформления записей и ссылок 
используются не `*.bst`, а `*.csl`-файлы.

Для нумерации объектов (теорем, рисунков) был написан маленький плагин `numbered-labels.rb` 
(пока дописывается и тестируется).
