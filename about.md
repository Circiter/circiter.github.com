...
пока нестабилен (по-умолчанию этот режим всё-равно отключен). N.B., плагин `numbered-labels.rb` 
обрабатывает исходный документ только один раз, повторный проход разбора не требуется, и, тем не 
менее, ссылки могут указывать как на объекты, определённые как выше, так и ниже по тексту.
...
уже было замечено выше, нумеровать их автоматически.
