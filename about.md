...
дважды (по-факту, трижды -- пакеты `mathtools` и `preview` плохо дружат), в то время как для 
режима `shared_context: false` достаточно одного прохода (но зато аж для всех формул... 
территория компромиссов указывает на оптимальность решения, иногда :) ).
