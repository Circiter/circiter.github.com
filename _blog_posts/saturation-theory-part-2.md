...
{% cite pomerance-argument %}. Сначала Карл Померанс замечает, что нечётное совершенное $n$ 
должно иметь вид $pm^2$, где $p\in\mathbb{P}\land p\equiv 1\pmod 4$. Т.к. $pm^2$ делится на $p$, 
то, учитывая определение совершенного числа, и сумма делителей должна делиться на $p$. Отсюда 
делается вывод об ограниченности количества таких $p$ логарифмом от $m$ и о том, что вероятность 
делимости $\sigma_1(pm^2)$ на $\pm^2$ равна $1/m^2$. Вероятность же того, что хотя-бы одно число 
$p$ подойдёт равна $\log(m)/m^2$. Из сходимости суммы $\sum\limits_m \log(m)/m^2$ делается вывод 
о существовании лишь конечного количества нечётных совершенных чисел.
...
сразу усиливает его до несуществования таких чисел вообще.
...
будет означать, что не существует ни одного нечётного совершенного числа. :)
...
$$\label{eq:phi-ineq}\min\limits_j\varphi(\mu_j)=3^1-3^0=3>2.$$
...
дробности выражения $$\label{eq:divisibility-check} \underbrace{\frac{r_j}{\mu_j} \left( 
