...
Я рад, что читатель не сбежал после прочтения аннотации. :) Да, проблеме о нечётных совершенных 
...
* $K=\proc_i k_i$ -- число, факторизация которого образована всеми числами $k_i$ (N.B., все 
факторы различны).
{% sentence_end  %}
...
* Число $x$ -- *слабо насыщенно* если $\forall p\in\mathbb{P}\ M\equiv 0\pmod p \Rightarrow 
x\equiv 0\pmod p$;
...
При каноническом порядке суммирования делителей, справедливо $\forall \Sigma_i\equiv 0\pmod 
...
Для нечётных $M$, при каноническом порядке делителей, ни одна из частичных сумм не достигает 
...
всех простых множителей в разложении $M$. Стрелками $\Longrightarrow$ помечены обычные 
логические импликации, $X\Rightarrow Y$ означает, что из утверждения $X$ следует утверждение 
$Y$.

Стрелки же $\longrightarrow$ (могущие быть заменёнными на $\Longleftarrow$) позволяют показать, 
как одно утверждение вносит вклад в доказательство другого -- $X\longrightarrow Y$ означает, что 
утверждение $X$, возможно наряду с другими утверждениями, <<вносит вклад>> в утверждение $Y$ -- 
к примеру, доказательство утверждения $X$ может входить, в качестве фрагмента, в доказательство 
$Y$; понятно, что утверждение $X$ в этом случае логически необходимо для утверждения $Y$, т.е. 
...
Т.к. $S\subseteq D$, то $\exists q\ \Sigma_q=\sum S$. Также, выполняется $K\not\equiv 
0\pmod k$, т.е. $k$ присутствует в разложении $K$ на простые множители. Все эти условия делают 
применимой гипотезу {% ref hypothesis main-observation %}. Она утверждает, что $\forall i\ 
\Sigma_i\equiv 0\pmod M \Rightarrow \text{НОД}(\Sigma_i, K)=1$. В частности, т.к. $k$ входит в 
факторизацию $K$, то $$\label{eq:saturated}\forall i\ \Sigma_i\equiv 0\pmod M \Rightarrow 
\Sigma_i\not\equiv k.$$
...
По прежнему $D$ -- множество делителей числа $M$. Тогда $$\sum D=\sigma_1(M) \label{sumdef}.$$ 
Заметим, что $M\in D$, ведь $M$ является своим делителем. При этом, очевидно, $$\max D=M 
\label{maxdef},$$ и, по построению, $$\forall d\in D\ \max D\equiv 0\pmod d. 
...
соответственно, ни один элемент множества $D$ тоже не кратен $k$; другими словами 
$$D\subset\mathbb{N}\setminus k\mathbb{N}. \label{setdivisibility}$$

Все эти определения и свойства, в частности, свойства $\eqref{maxdivisibility}$ и 
$\eqref{setdivisibility}$, означают, что ко множеству $D$, при данных $n$ и $k$, применима 
гипотеза {% ref hypothesis auxiliary-hypothesis %}. Её заключение утверждает, что $\sum D\neq 
k\max D$, откуда следует (учитывая равенства $\eqref{sumdef}$ и $\eqref{maxdef}$), что 
...
Число $n$, такое, что $\sigma_1(n)=2n$, называется \emph{совершенным} числом (половина суммы 
всех делителей совершенного числа равна самому числу).
{% sentence_end %}

{% sentence_begin %}
**Гипотеза {% def hypothesis no-odd-perfect %}**

$\sigma_1(p)=2p\Rightarrow p\equiv 0\pmod 2$. Т.е. предполагается, что не существует нечётных 
совершенных чисел.
{% sentence_end %}
...
аж к Никомаху Герасскому (*circa* 100 лет до н.э.).), см. переводы {% cite nicomachus-ru %} и {% 
cite nicomachus-en %}. Более того, сам я точно не знаю, в какой именно из своих работ Эйлер 
...
В XIX веке, Сильвестер приводил известный эвристический аргумент {% cite sylvester1887 %} в 
пользу несуществования нечётных совершенных чисел. Он аппелировал к слишком малой вероятности 
одновременного удовлетворения нечётным совершенным числом всем известным ограничениям (а их 
накопилось немало) на возможный вид таких гипотетических объектов.

В следующем, XX веке появился теперь широко известный эвристический аргумент Померанса {% cite 
pomerance-argument %}. Сначала Карл Померанс замечает, что нечётное совершенное $n$ должно иметь 
вид $pm^2$, где $p\in\mathbb{P}\land p\equiv 1\pmod 4$. Т.к. $pm^2$ делится на $p$, то, учитывая 
определение совершенного числа, и сумма делителей должна делиться на $p$. Отсюда делается вывод 
об ограниченности количества таких $p$ логарифмом от $m$ и о том, что вероятность делимости 
$\sigma_1(pm^2)$ на $\pm^2$ равна $1/m^2$. Вероятность же того, что хотя-бы одно число $p$ 
подойдёт равна $\nicefrac{\log(m)}{m^2}$. Из сходимости суммы $\sum_m\nicefrac{\log(m)}{m^2}$ 
делается вывод о существовании лишь конечного количества нечётных совершенных чисел.
...
рассматривать сумму $\sum_{m\colon m>10^75}\nicefrac{\log(m)}{m^2}$ (как я понимаю, из-за того, 
что $10^75=\sqrt{10^150}$) и, численно оценив эту сумму как не превышающую $10^-70$, делает 
заключение о слишком малой вероятности существования нечётных совершенных чисел и даже сразу 
усиливает его до несуществования таких чисел вообще.
...
Я собираюсь применить строящуюся теорию насыщения к гипотезе {% ref hypothesis no-odd-perfect 
%}. :)
...
произвольности выбора нечётного $n$ следует, что ни одно нечётное $n$ не может удовлетворять 
уравнению $\sigma_1(n)=2n$, т.е. не может быть совершенным. Иначе говоря, все совершенные числа 
являются чётными. $\blacksquare$

Эйлер показал, что у нечётных совершенных чисел один фактор должен иметь первую степень, 
...
будет означать, что ни одно нечётное совершенное число не существует. :)
...
Сам срок, прошедший с выхода <<Введения в арифметику>> Никомаха из Герасы, превысивший уже 
две тысячи лет, намекает на некоторую подозрительность моего результата. Но сам феномен 
насыщения не становится менее интересным.

Вопросы и задачи, остающиеся пока открытыми:

* Верна ли гипотеза {% ref hypothesis general-conjecture %}?
...
сделав теория насыщения достаточно формальной для применения <<хотя бы>> к проблемы нечётных 
простых чисел.
...
* Функция Эйлера мультипликативна: $$\label{eq:euler-multiplicativity} 
\varphi(ab)=\varphi(a)\varphi(b);$$
* Для $p\in\mathbb{P}$ верно $$\label{eq:euler-prime} \varphi(p)=p-1;$$
...
В общем случае, число $M$ факторизуется в виде $M=\proc_j m_j^{\alpha(j)}$ (каноническое 
...
\right)^{\varphi(\mu_j)} \cdot(-r_j),$$ заменяющем решение $\eqref{eq:generalsolution}$.
...
$\min_j\alpha(j)=1$, а из-за нечётности $M$ выполняется $\min_j m_j = 3$. Отсюда, 
$$\label{eq:phi-ineq}\min\varphi(\mu_j)=3^1-3^0=3>2.$$

Здесь будут пропущены рассуждения, аналогичные рассуждениям из первого сообщения (выполненным 
для $m_j$, а не для $\mu_j$), приводящие к необходимости проверки делимости выражения 
$\eqref{eq:generalsolution}$ на модули $\mu_j$. Из этих рассуждений вытекает задача проверки 
дробности выражения $$\label{eq:divisibility-check} \underbrace{\left( \nicefrac{M}{\mu_j} 
\right)^{\varphi(\mu_j)} \frac{r_j}{\mu_j}}_\text{I} + \underbrace{\sum_{h\colon h\neq j} 
\left(\nicefrac{M}{\mu_h}\right)^{\varphi(\mu_h)} \frac{r_h}{\mu_j}}_\text{II},$$ полученного из 
...
даст целое число, большее единицы. Единица могла бы получиться только если в наличии имеется 
только один модуль $\mu_h$, но в таком случае всё подвыражение $\text{II}$ было бы нулём (см. 
предыдущий абзац), а значит -- целым числом. Если же дробь $\nicefrac{M}{\mu_h}>1$, то после 
возведения в степень получится целое число, делящееся на $\mu_j$ (т.к. $j\neq h$). Это приводит 
к целочисленности подвыражения $\text{II}$.

В подвыражении $\text{I}$ дробь $\nicefrac{M}{\mu_j}$ -- целая и больше единицы (аргументация та 
же, что и выше). Но важно, что такая дробь <<вычеркивает>> $\mu_j$ из $M$, и, поэтому, сама 
дробь на $\mu_j$ уже не делится. Результат возведения в степень $\varphi(\mu_j)$ тоже не будет 
делиться на $\mu_j$. Число $r_j$, опять по аналогии с предыдущим сообщением, определим как 
остаток от деления частичной суммы $\Sigma_i$ на модуль $\mu_j$. Такой остаток меньше $\mu_j$ и 
делиться на $\mu_j$ не может. В итоге получается, что подвыражение $\text{I}$ -- есть дробь со 
знаменателем $\mu_j$ и числителем, не делящемся на $\mu_j$. Т.е. подвыражение $I$ -- дробное 
число.
