...
гипотезу {% ref hypothesis general-conjecture %}), утверждающей, что из насыщенности следует 
...
доказательства гипотезы {% ref hypothesis odd-no-perfect %} напрямую (т.е., на текущий момент, 
...
$s\in\mathfrak{S}$ обозначим $W_s=\prod\limits_{\mu\in s} \mu=\prod s$. Каждая система 
$s\in\mathfrak{S}$ порождает подмножество $D_s\subseteq D$, такое, что каждый делитель $d\in 
D_s$ конструируется из этих модулей, т.е. $\exists s'\subseteq s\ d=\prod s'$.
{% sentence_end %}
