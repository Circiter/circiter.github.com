...
Пусть $\mathfrak{S}$ -- семейство всех возможных систем модулей (для данного числа $M$). 
Для краткости, обозначим $W_s=\prod\limits_{\mu\in s} \mu=\prod s$. Каждая 
система $s\in\mathfrak{S}$ порождает подмножество $D_s\subseteq D$, такое, что каждый делитель 
...
Подразумевается, что для любой $s\in\mathfrak{S}$ действительна прямая адаптация аргумента, 
...
число $d'$, оказывается некратным ни одному из модулей $\mu\in\xi(i)$. Из гипотезы {% ref 
