...
toc: true
xdate: "август, 2020"
tags: программирование математика эзотерика sed теория-чисел trivia
---
...
поисковых машинах интернета не дал конкретных результатов, поэтому я решил написать этот
скрипт самостоятельно, а в этой заметке рассказать о его работе.
...
$\mathcal{M}_{i+1}=\{m\in M_{i+1} | m>p_i\}$. Если же $\mathcal{M}=\varnothing$, то процесс 
...
Возможно имеется смысл немного детализировать работу решета на этапе поиска следующего простого 
числа. В частично заполненном решете отыскивается первая единица, лежащая правее маркера `#`, 
после чего этот маркер перемещается на позицию сразу после этой единицы. Длина части решета, 
лежащей левее маркера есть новое простое число. Этот префикс копируется на следующую строчку и 
может считаться записью найденного простого числа в унарной системе счисления.
...
входящее в этого решето (и только они) будет входить в разложение данного числа на простые 
множители.
...
текущего простого числа (не выходищих за пределы решета, естественно) значение счётчика будет 
...
Второй скрипт, `collatz.sed`, реализует известную динамическую систему из гипотезы $3x+1$ [3], 
но вместо привычных операций $3x+1$ и $x/2$ используются манипуляции с простыми числами, на мой 
взгляд лучше показывающие истинную теоретико-числовую суть гипотезы Коллатца.
...
* `anagram.sed` и `collatz.sed` были описаны в предыдущем разделе этого сообщения. Скрипт 
  ...
  утроен и инкрементирован, что даст 16 ($10000_2$), а это последнее число будет сразу же 
  [целочисленно] разделено на максимально возможную степень двойки, $2^4$, что даст последнюю 
  точку траектории, 1.
...
* `factor-hack-unary.sed` -- простой скрипт, вместо решета использующий <<обратные ссылки>> в
  регулярном выражении (следуя [4, 8]) для факторизации числа, представленного в унарной системе 
  счисления.
  
  Примечание: здесь используются расширенные <<регулярные>> выражения, поддерживающие ссылки 
  вида `\1`, `\2` etc., и не являющиеся именно регулярными в привычном смысле -- такие выражения 
  с обратными ссылками не удастся скомпилировать в конечный автомат.
  ...
  Для числа $n$ используется регулярное выражение, аналогичное выражению `/^(11+)(\1+)$/`. Как 
  видно, оно выделяет некую группу из не менее чем двух единиц при помощи подвыражения `(11+)`, 
  т.е. выделяет один множитель $a$ (возможно составной), и с помощью подвыражения `(\1+)` 
  проверяет, можно ли представить оставшуюся часть числа конкатенацией некоторого ненулевого 
  количества копий этой группы (без остатка).
  ...
  жадностью -- так в источнике [4] вместо `11+` применено подвыражение `11+?`, которое как раз и 
