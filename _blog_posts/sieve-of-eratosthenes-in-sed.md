...
[Модифицированное] решето Эратосфена может применяться для вычисления некоторых функций, 
хорошо известных в теории чисел. Примеры таких функций {% cite versatile-sieve %}:
- Простота числа; основное применение обычного решета Эратосфена.
- Радикал целого числа (функция $\operatorname{rad} n$); $\operatorname{rad} n$ равен 
  произведению всех различных простых делителей числа $n$.
- Функция Эйлера ($\varphi$-функция); $\varphi(n)$ равно количеству чисел, взаимно простых с 
  $n$.
- Количество различных простых делителей ($\omega$-функция).
