...
общего делителя, $\operatorname{gcd}(x, 3x+1)=1$. Добавление же тройки гарантирует, что в 
...
p\in\mathbb{P}_{\leqslant h} \operatorname{gcd}(m, p)=1$ для некоторого $h\geqslant 2$; e.g., 
при $h=2$ это соответствует игнорированию всех чётных чисел, больших двух, всегда по-определению 
являющихся составными.
...
образуют множество $[p; \lfloor n/p\rfloor]$ (максимум здесь выбран так, чтобы после умножения 
на $p$ получилось $p\lfloor n/p\rfloor\leqslant n$, а минимум соответствует началу вычеркивания 
с $p^2$).
...
Суть в том, что любое натуральне число $x$ можно единственным образом представить в виде $x=pf$, 
где $p$ -- наименьший простой делитель (фактор) числа $x$. При этом среди факторов числа $f$, 
очевидно, не будет чисел, меньших $p$. (Т.е. мы просто <<отсоединяем>> наименьший фактор $p$ от 
$x$ и формируем разложение $x=pf$ полагая $f=x/p$; тогда из основной теоремы арифметики [6], 
т.е. из единственности факторизации как раз и следует единственность требуемого разложения 
$pf$.)
...
числа, подлежащие вычеркиванию, равно $L(p)=\{f\colon f\in[p; n], \forall q<p\ 
\operatorname{gcd}(q, f)=1\}$.

Покажем, что такое определение $\mathcal{L}(p)$ совпадает с $L$ из алгоритма (см. выше). На 
каждой итерации, если $p=p_i$, то $\forall x\in L\ j<i\Rightarrow \operatorname{gcd}(x, p_j)=1$, 
т.е. все числа из $L$, большие чем $p_i$ не делятся на простые меньшие $p_i$ ведь составные 
числа, делящиеся на $p_i$ уже были вычеркнуты на предыдущих итерациях алгоритма. (Здесь идет 
речь об $L$ до шага 5, т.е. до операции $L\gets L\setminus N$.)

Т.о., $L$ содержит числа из промежутка $[p; n]$, не делящиеся на простые, меньшие $p$, т.е. 
$L=\{f\colon f\in[p; n], \forall q<p\ \operatorname{gcd}(q, f)=1\}$. Это совпадает с выражением 
для $\mathcal{L}(p)$.
...
требуемых операций нужно проссумировать это выражение по всем простым, не превышающим $n$, т.е. 
асимптотическая сложность равна $\sum\limits_{p\in\mathbb{P}_{\leqslant n}}$. Известно, что 
$\sum\limits_{p\in\mathbb{P}_{\leqslant n}}\frac1p = \log\log n +\mathcal{O}(1)$. Таким образом, 
временн&#x301;ая сложность решета Эратосфена оценивается как $\mathcal{O}\big(n\log\log n\big)$, 
где $\log\log n$ выполняет роль среднего количества различных делителей числа $n$. (N.B., за 
$\mathcal{O}$-нотацией ещё прячется слагаемое $\mathcal{O}(n)$, т.е. время, требуемое на 
инициализацию решета построением множества $[2; n]$.)
...
оперативной памяти, а на носителе типа flash-микросхем, имеющим ограничение на количество 
...
простом числе без шанса на попадание в <<своих>> (в простые числа) и без возможности тратить 
<<боеприпасы>> и время впустую для попыток вычеркивания уже вычеркнутых чисел.
...
сразу после `#`, то это означает, множителем будет само простое число, т.е. это соответствует 
вычислению $p^2$). Маркер движется вправо и устанавливается на следующее невычеркнутое число.
...
маркер `:` перед первым префиксом и `,` -- перед вторым. Оба префикса т.о. всего-лишь обозначают 
подлежащие перемножению числа [в унарной системе счисления]. Их произведение следует вычеркнуть 
из решета.
...
аналогичном этами производилось, собственно, вычеркивание командой `s/1@/0@/`, то сейчас 
...
маркера `#`. Если правее от маркера `#` не оказывается простых чисел, т.е. нет ячеек со 
