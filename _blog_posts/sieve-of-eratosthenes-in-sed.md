...
общего делителя, $\operatorname\text{НОД}(x, 3x+1)=1$. Добавление же тройки гарантирует, что в 
...
p\in\mathbb{P}_{\leqslant h}\ \operatorname\text{НОД}(m, p)=1$ для некоторого $h\geqslant 2$; 
...
\operatorname\text{НОД}(q, f)=1\}$.

Покажем, что такое определение $\mathcal{L}(p)$ совпадает с $L$ из алгоритма (см. выше). На 
каждой итерации, если $p=p_i$, то $\forall x\in L\ j<i\Rightarrow \operatorname\text{НОД}(x, 
...
$L=\{f\colon f\in[p; n], \forall q<p\ \operatorname\text{НОД}(q, f)=1\}$. Это совпадает с 
...
(Интересно, что в некоторых работах, например, в той же [13], прямые отсылы к Эйлеру 
отсутствуют, поэтому иногда говорят, что это решето неоднократно переизобреталось различными 
авторами.)
