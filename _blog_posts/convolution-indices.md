...
В настоящем сообщении я хотел бы показать естественность свертки используя в ряд простых примеров,
...
важное свойство реальных систем, а именно независимость результатов эксперимента от времени его начала: если мы опоздали с
началом эксперимента на некоторое время $\tau$, то и результаты мы получим с опозданием
$\tau$. Заметьте, что здесь отклик меняется также как и воздействие, поэтому более правильно
говорить не об инвариантности, а о <i>ковариантности</i> по времени.
...
$$\delta(t)=\begin{cases}+\infty, & x=0\\0,\end{cases}$$ интеграл по всему пространству равен 1:
$$\int\limits_{-\infty}^\infty\delta(t)\mathrm{d}t=1.$$ Причем из этого уравнения следует
<i>фильтрующее свойство</i> $\delta$-функции:
$$\int\limits_{-\infty}^\infty f(\tau)\delta(\tau-t)\mathrm{d}\tau=f(t).\eqno (11)$$

Доказательство фильтрующего свойства.

Т.к. $\delta(\tau-t)=0$ при $\tau\neq t$, то интеграл (11) не зависит от значений $f(\tau)$ при
$\tau\neq t$, а зависит только от значения в единственной точке $f(\tau=t)$. Поэтому функцию $f(\tau)$
можно просто заменить константой $A=f(t)=\mathrm{const}$, которую можно будет вынести за знак интеграла:
$$A\int\limits_{-\infty}^\infty \delta(\tau-t)\mathrm{d}\tau=f(t).$$ Но теперь оставшийся интеграл
равен единице и (11) превращается в тождество $A=f(t)$. $\blacksquare$
...
отдаленной от <<бытового>> применения математики; а аналог теоремы о свертке в случае с корреляцией
требует использования  лишней операции комплексного сопряжения.
