...
$$h_i=\sum_{j=0}^{n-1} a_j b_{(i-j)\mod n}\eqno{(1)}.$$
...
$f(x)\ast g(x)=\int_{-\infty}^\infty f(y)g(x-y)\mathrm{d}y$. (N.B., этот интеграл
тоже является функцией от $x$.)
...
$b=\sum_{i=0}^mb_i10^j$,
соответственно, где $a_i, b_i\in[0;9]\subset\mathbb{N}$. Результат будет равен $$c=ab=\sum_i\sum_j a_ib_j 10^{i+j}.\eqno{(4)}$$
...
$$(f\star g)(x)\equiv\int_{-\infty}^\infty \overline{f(y-x)}g(y)\mathrm{d}y.$$
...
$$(f\star g)(x)\equiv\int_{-\infty}^\infty \overline{f(y)}g(y+x)\mathrm{d}y.$$
...
Как видно и вышеприведенных примеров, свертка является достаточно естественной концепцией,
