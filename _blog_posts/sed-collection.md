...
широко известного поточного редактора `sed`, в работе которого основное место занимает команда
...
историю существования этого редактор, использовавшегося с начала/серидины шестидесятых годов
...
Под впечатлением от однострочной C-программы некоего Эрика Копчинского (Eryk Kopczynski),
написанной для небезызвестного конкурса ioccc (см. [http://ioccc.org]) за 2004 год и выполняющей
оптическое распознавание текста находя эйлерову характеристику его "триангуляции" (насколько
...
топологического инварианта, очевидно одинакового вообще для всех планарных графов; поэтому
интересующихся пока переадресую на статью S.B.Gray, Local Properties of Binary Images in
Two Dimensions, 1971, предположительно являющуюся основой для программы Копчинского. По-сути,
авторы расчитывают инвариант, равный разности между количеством связных областей и количеством
дырок, но расчитывают его подсчитывая различные чисто локальные конфигурации пикселей
в $2\times 2$-окне, пробегая им по всему изображению (в статье этот сверточный подход распространен
и на $3\times 3$-окна, но у Копчинского применено маленькое окно).
...
`sudo cat /dev/mem > /dev/audio`? Редактор `sed` вполне может приням со стандартного ввода
...
Генерация [вообще говоря, неминимальных] [суперперестановок](http://en.wikipedia.org/wiki/superpermutation).
...
Д.Лукаса. Применен тот же принцип герерации текстового потока, озвучиваемого перенаправлением в
`/dev/audio`. В отличии от программы-пианино или от генератора "морзянки", здесь кроме чистых нот
(на самом деле всего-лишь прямоугольного сигнала) имеется возможность создания белого шума,
а также случайного выбора нот.

Интересно, можно ли научить `r2d2` говорить [по-человечески]?
...
* [sliding-window.sed](https://gist.github.com/Circiter/77951d75b6b0e3b4ecc9b689e17863c3) -- другой
   ...
   что один (левый) маркер находится над (т.е. выше) текущей клеткой, правый -- под ней. Далее вокруг
   текущей клетки выделяется окрестность $3x3$, окрашивается, отображается. И так для всех клеток с их
   ...
   вверх-вниз вдоль маркеров, затем удалить их и работать с другой клеткой (счетчик не нужен). Элементарно.

<i>Nota bene,</i> я почти всегда пишу на диалекте [`GNU sed`](http://www.gnu.org/software/sed), что, увы,
создаёт проблемы с использованием моих `sed`-скриптов на всяких там Mac'ах, BSD'ях и этих ваших
Android'ах. Возможен ли конвертер (может быть даже написанный, о ужас, на самом `sed`) из GNU в
Posix и решило бы это проблему портабельности? Ближайший по смыслу найденный мною
проект -- [lvogel.free.fr/sed/sedcheck.sed] за авторством [Laurent Vogel](http://lvogel.free.fr),
проверяющий некоторый скрипт на Posix-совместимость.
...
На страницах [http://sed.sourceforge.net] и [http://sed.sourceforge.net/grabbag] (автор: Paolo Bonzini) можно найти
неплохую коллекцию `sed`-скриптов разной степени полезности. Стоить отметить некоторые из
...
* [http://sed.sourceforge.net/grabbag/scripts/turing.sed] -- машина Тьюринга на `sed`; да,
   это -- по-сути доказательство вычислительной универсальности языка `sed`.
* [http://sed.sourceforge.net/grabbag/scripts/sokoban.sed] -- широкоизвестная игра
   倉庫番(Sokoban, Сокобан); эта реализация интересна, разумеется, перемещением персонажа во всех
...
* [sedlisp](https://github.com/shinh/sedlisp) -- написанный японцем Shinichiro Hamaji интерпретатор небольшого подмножества lisp'а.
...
* [`quine.sed`](https://github.com/MakeNowJust/quine/blob/master/quine.sed) из
   ["музея квинов"](https://github.com/MakeNowJust/quine) -- квин на `sed`; самостоятельно его написать не так-то и просто,
   но у его [автора](http://quine.codes/) богатый опыт в квинах (если я правильно понял, им было написано более
   260 квинов на разных языках программирования).
