...
Тьюринг-полноты входного языка sed, вместе с метками и коммадами перехода (условного и безусловного)
всё же делает данный редактор вычислительно-универсальным. Фактически, он может рассматриваться в
качестве реализации [нормальных алгорифмов Маркова][nma], а полнота по-Тьюрингу обуславливает саму
возможность всех произошедших с sed приключений [с его нестандартным использованием].
...
Даже будучи <i>текстовым</i> редактором, sed не предоставляет удобных инструментов для разбора
...
Конкретно этот проект является маленьким (и страшн неэффективным) компилятором формул, принимающим
...
Perl-скриптом [m2elf.pl](https://github.com/stedolan/m2elf).
...
проект -- <http://lvogel.free.fr/sed/sedcheck.sed> за авторством [Laurent Vogel](http://lvogel.free.fr),
...
* <http://sed.sourceforge.net/grabbag/scripts/sokoban.sed> -- широкоизвестная игра
   ![i18n-sokoban](http://circiter.github.io/public/images/sokoban.png){:.inline} (сокобан);
...
* [sedtris](https://github.com/uuner/sedtris) -- реализация игры <<тетрис>>, выполненнная [Юлией Йомантайте](https://github.com/uuner) (из Нью-Йорка).
...
* <http://laurent.le-brun.eu/pub/path.sed> -- другая, более минималистичная, реализация обхода
   лабиринтов, написанный [Laurent Le Brun](http://laurent.le-brun.eu).
...
* <https://github.com/mame/quine-relay> -- <<итеративный>> многоязыковой квин, (авторы: [Yusuke Endoh](https://github.com/mame),
   даже написавший [книжку](http://gihyo.jp/book/2015/978-4-7741-7643-7), на японском, об эзотерическом
   программировании вообще и квинах в частности; и [hirekoke](https://twitter.com/hirekoke)),
   а именно программа, написанная на одном языке, при запуске выдающая программу
   на другом языке, в свою очередь печатающую программу на третьем, и т.д., пока последняя программа не
   напечатает снова самую первую. Конкретно этот многоязыковой квин, пробегает
   при своей работе 128 языков (на начало 2020 года), в том числе и sed.

   К сожалению, хотя такие <<квины>>, на первый взгляд, могут показаться достаточно сложными, на деле
   же сложность их написания оказывается несоизмеримо меньше сложности обычных, истинных квинов.
   Также не следует путать такие программы с квинами-полиглотами (квин-полиглот, или, как я его кратко
   называю, поликвин, является корректной программой одновременно на нескольких языках) и мультиквинами
   (программами, печатающими подобные программы на разных языках в зависимости от аргументов
   коммандной строки, в т.ч. ведущими себя как обычные квины будучи запущенными без аргументов).

   (Термины <<итеративный квин>>, <<поликвин>>, <<мультиквин>> нельзя назвать общепринятыми и
   они используются здесь просто чтобы дать соответствующим понятиям подходящие имена;
   официальной квинологической терминологии, боюсь, пока нет.)
