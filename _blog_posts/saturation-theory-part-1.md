...
N.B. $\left(\frac{M}{m_j}\right)$ -- это просто дробь в скобках, а не какой-нибудь там символ 
Лежандра. :)
...
\underbrace{\sum_{h\colon h\neq j} \left(\frac{M}{m_h}\right)^{m_h-1} r_h 
\frac{1}{m_j}}_\text{II}.$$
...
$$\left(\frac{M}{m_j}\right)^{m_j-1}\frac{r_j}{m_j} = \frac{r_j}{m_j}\left(\prod_{q\colon q\neq 
j} m_q\right)^{m_j-1}.$$ В произведении, возведённом в степень, можно перегруппировать 
