...
использует в своей факторизации все факторы числа $M$, но, возможно, в других количиствах, и, в 
...
в исходном числе $M$. Синий цвет имеют факторы присутствующие в $M$, а также факторы, 
находящиеся и в $M$ и в частичной сумме одновременно, но не являющиеся критически важными для 
насыщенности. Красным цветом обозначены факторы частичной суммы, общие с факторизацией $M$ и 
важные для присвоения той или иной степени насыщенности данной частичной сумме. Сумма будет 
насыщенной если в ней есть все красные факторы из числа $M$, т.е. если факторизация $M$ входит в 
факторизацию $\Sigma_i$ в качестве подпоследовательности. Сумма $\Sigma_i$ будет сильно 
насыщенной если она насыщенна и не имеет жёлтых факторов. Вот некоторые примеры:

{% tex inline %}
{% raw %}
\gdef\drawarray#1#2{%
    \makeatletter\@ifundefined{c@xcounter}{\newcounter{xcounter}}{}\makeatother
    \setcounter{xcounter}{0}
    \foreach \p/\c in {#1}
    {
        \if k\c
            \def\rcolor{yellow!70!white}
        \else
            \if n\c
                \def\rcolor{blue!30!white}
            \else
                \def\rcolor{red!30!white}
            \fi
        \fi
        \def\position{(.65*\thexcounter,0)}
        \draw[\rcolor,fill=\rcolor] \position circle (.3);
        \draw \position node {\p};
        \global\stepcounter{xcounter}
    }
    \draw (.65*\thexcounter-.4,0) node[right] (a) {#2};
}
\gdef\scheme#1#2#3#4{%
    \left\{\begin{array}{l}
        \mbox{\tikz\drawarray{#1}{$#2$};}\\
        \mbox{\tikz\drawarray{#3}{$#4$};}
    \end{array}\right.}

$\scheme{2/n,3/n,3/n}{=M=18}{2/m,2/n,3/m}{=\Sigma_4=12}\ 
\scheme{2/n,2/n,5/n}{=M=20}{7/k}{=\Sigma_3=7}$
{% endraw %}
{% endtex %}

$\scheme{3/n,7/n}{=M=21}{2/k,2/k,2/k,2/k,2/k}{=\Sigma_4=32}\ 
\scheme{2/n,2/n,2/n,3/n}{=M=24}{2/m,2/m,3/m,5/k}{=\Sigma_8=60}$

$\scheme{2/n,2/n,2/n,3/n}{=M=24}{2/m,2/m,2/m,3/m}{=\Sigma_6=24}\ 
\scheme{2/n,2/n,2/n,3/n,5/n}{=M=120}{2/m,2/m,2/m,2/n,3/m,5/m}{=\Sigma_{14}=240}$

Обратите внимание, как только факторизация $M$ обнаруживается входящей полностью в факторизацию 
некоторой $\Sigma_i$, жёлтые факторы гарантированно вытесняются из факторизации такой частичной 
суммы (это показано в двух последних примерах; в примере с $M=18$, несмотря на слабое насыщение 
частичной суммы $\Sigma_4$, в ней тоже нет жёлтых факторов, но это лишь совпадение -- см. пример 
с $M=24, \Sigma_8=60$).
...
В контексте разбора случая факторизуемости $M$ без повторов, стоит уделить особое внимание 
...
{% sentence_begin %}
**Предложение:** $\forall x, a, b$ верно $(x\bmod a)\bmod b=x\bmod\text{НОД}(a, b)$.
{% sentence_end %}
