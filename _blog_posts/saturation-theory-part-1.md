...
насыщенной если она насыщенна и не имеет жёлтых факторов. Вот эти примеры:

{% tex inline %}
{% raw %}
\makeatletter
\gdef\usecounter#1{\@ifundefined{c@#1}{\newcounter{#1}}{}}
\makeatother
...
\gdef\drawarray#1#2{%
    ...
    \foreach \p/\c in {#1}
    {
        ...
        \else
            ...
            \fi
        \fi
        \def\position{(.5*\thexcounter,0)}
        \draw[\rcolor,fill=\rcolor] \position +(-.2,-.2) rectangle +(.2,.2); %circle (.3);
        \draw \position node {\p};
        \global\stepcounter{xcounter}
    }
    \draw (.5*\thexcounter-.3,0) node[right] (a) {#2};
}

\gdef\xscheme#1#2#3#4{%
    \left\{\begin{array}{l}
        \mbox{\tikz\drawarray{#1}{$#2$};}\\
        \mbox{\tikz\drawarray{#3}{$#4$};}
    \end{array}\right.}

$\xscheme{2/n,3/n,3/n}{=M=18}{2/m,2/n,3/m}{=\Sigma_4=12}$ 
{% endraw %}
{% endtex %}

$\xscheme{2/n,2/n,5/n}{=M=20}{7/k}{=\Sigma_3=7}$ 
$\xscheme{3/n,7/n}{=M=21}{2/k,2/k,2/k,2/k,2/k}{=\Sigma_4=32}$ 
$\xscheme{2/n,2/n,2/n,3/n}{=M=24}{2/m,2/m,2/m,3/m}{=\Sigma_6=24}$ 
$\xscheme{2/n,2/n,2/n,3/n,5/n}{=M=120}{2/m,2/m,2/m,2/n,3/m,5/m}{=\Sigma_{14}=240}$
