...
b_j)=1$. Тогда система сравнений $x\equiv a_i\pmod b_i,\ \forall i$ имеет ровно одно решение 
$x\equiv x'\pmod B$, где $B=\prod_{i=1}^n b_i$ и $$x'=\sum_{i=1}^n a_i\left( 
\nicefrac{B}{b_i}\right)^{\varphi(b_i)}.$$
{% sentence_end %}

Т.к. $\varphi(x)$ равна количеству чисел, меньших $x$ и взаимно простых с $x$, то при 
...
statement prime-case %}), то у числа $M$ больше двух простых делителей и больше двух делителей. 
Последнее обстоятельство приводит к неравенству нулю обоих подвыражений $\text{I}$ и 
$\text{II}$ (хотя, обращаться к утверждению {% ref statement prime-case %} необязательно -- 
если $M\in\mathbb{P}$, то подвыражение $\text{II}$, и только оно, обратится в ноль, но т.к. 
ноль есть число целое, то подвыражение $\text{II}$ всё-равно никак не повлияет на дробность 
или целочисленность всей суммы подвыражений $I$ и $II$).
...
$M$), то $(\nicefrac{M}{m_h})^{m_h-1}>1$ и один из этих $m_h-1$ множителей можно будет поделить 
на $m_j$, в результате чего в знаменателе вообще ничего не останется, и текущее слагаемое, --- а 
значит и вся сумма $\text{II}$, --- будет целым числом, из-за чего подвыражение $\text{II}$ 
