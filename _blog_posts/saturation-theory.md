...
$$\label{eq:condition}\forall j\ -\sum_h\left(\frac{M}{m_h}\right)^{m_h-1}r_h\not\equiv 
...
Что касается первого вопроса (о выполнении условия $\eqref{eq:condition}$), то можно попробовать 
рассмотреть игрушечный случай с лишь двумя факторами в числе $M$. Итак, пусть $M=m_0m_1$ при 
$m_0\neq m_1$ и $m_0, m_1>2$. Ограничимся проверкой условия $\eqref{eq:condition}$ для $m_j=m_0$ 
...
остаётся в числителе, делая эту дробь целой и сводя проверку условия $\eqref{eq:condition}$ к 
...
$\eqref{eq:condition}$, разделим его на $m_j$ и разложим сумму, выделим в неё одно слагаемое: 
